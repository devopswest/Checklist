...
if [ "$DOCKER_TARGET" = "" ]; then
    ...
    name="$(docker -H $DOCKER_TARGET service inspect todos|grep Image)"
    ...
    if [ "$x1" = "[]" ]; then
        ...
        docker -H $DOCKER_TARGET service create --replicas 1 -p 8080:8080 -e SERVICE_PORT=8080 -e SERVICE_ENV=dev -e SERVICE_ES_CLUSTER=elastichsearch -e SERVICE_ES_NODE=pirates-dtr.eastus.cloudapp.azure.com:9300 --name $appname $DOCKER_USER/$appname:$branch
